...
import { MailTemplateService, EmailTemplateData } from './main-template';
...
export class MailService {
  ...
  async sendPasswordResetEmail(email: string, resetToken: string, userName?: string) {
    ...
    const html = this.mailTemplateService.generatePasswordResetEmail(resetUrl, userName);

    return await this.sendEmail({
      to: email,
      subject: 'Reset Your Password - IntelliScanAI',
      html,
    });
  }
}