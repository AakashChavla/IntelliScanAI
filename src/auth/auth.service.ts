...
export class AuthService {
    ...
    async forgetPassword(dto: forgotPasswordDto, res: Response) {
        try {
            const { email } = dto;
            const user = await this.databaseService.user.findUnique({
                where: { email },
            ...
            const token = this.jwtService.sign(
                { id: user.id, email: user.email, purpose: 'password-reset' }, // <-- add purpose
                { expiresIn: '1h' },
            );

            // Send the reset email
            await this.mailService.sendPasswordResetEmail(user.email, token, user.name ?? undefined);
    ...
    async resetPassword(dto: resetPasswordDto, email: string, res: Response) {
        try {
